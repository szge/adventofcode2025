 ...
 * @return
 */
int count_valid(const std::vector<bool>& grid, const std::pair<int, int> grid_dims)
{
    ...
    for (int y {0}; y < height; y++)
    {
        ...
        {
            if (grid[y * width + x]) num_valid += is_valid(grid, grid_dims, {x, y});
        }
    }
    return num_valid;
}
...
int main () {
    ...
    std::println("The solution is {}", count_valid(grid, {width, height}));
